...
function mod10Check(numStr){
  const s=String(numStr||""); if(!/^\d+$/.test(s)) return false;
  const ds=s.split("").map(Number);
  const check=ds[ds.length-1];
  const body=ds.slice(0,-1);
  let sum=0,w=3;
  for(let i=body.length-1;i>=0;i--){ sum+=body[i]*w; w=(w===3)?1:3; }
...
/* ========= state ========= */
function defaultState(){ return { drafts:[], done:[], docsByDoctor:{}, scanLog:[] }; }
...
/* ========= data ========= */
const FALLBACK_OPERATORS=[{id:"OP1",label:"看護師A"},{id:"OP2",label:"CE技士A"}];
const FALLBACK_PATIENTS=[{id:"PT1",label:"患者001"},{id:"PT2",label:"患者002"}];
const FALLBACK_PROCEDURES=[{id:"PR-CATH-001",label:"冠動脈造影（CAG）"},{id:"PR-PCI-001",label:"PCI"}];
const FALLBACK_DOCTORS=[{id:"DR1",name:"医師A",dept:"循環器内科"}];
const FALLBACK_BILLMAP={byTokuteiName:{},byProductName:{}};
const FALLBACK_STANDARD_BUILDER={
  version:"fallback",domain:"cathlab",
  defaultProcedureCandidates:["PR-CATH-001","PR-PCI-001"],
  rules:[{name:"ステント",matchAny:["ステント","DES"],suggest:["PR-PCI-001"]}]
};
...
/* ========= label helpers ========= */
const operatorLabel=(id)=>(OPERATORS.find(x=>x.id===id)?.label)|| (id||"未選択");
const patientLabel =(id)=>(PATIENTS.find(x=>x.id===id)?.label)|| (id||"未選択");
const procedureLabel=(id)=>(PROCEDURES.find(x=>x.id===id)?.label)|| (id||"未選択");
const doctorDeptList=()=>Array.from(new Set(DOCTORS.map(d=>d.dept).filter(Boolean))).sort();
const doctorLabelById=(id)=>{
  if(!id) return "未選択";
...
/* ========= recent approver ========= */
function touchRecentApprover(id){ if(!id) return; recentApprovers[id]=Date.now(); save(); }
function sortedApprovers(dept){
  const list=DOCTORS.filter(d=>!dept||dept==="ALL"||d.dept===dept).slice();
  list.sort((a,b)=>{
    const ta=recentApprovers[a.id]||0, tb=recentApprovers[b.id]||0;
...
/* ========= docs (doctor) ========= */
const doctorKey=()=>`${(doctorProfile.dept||"").trim()}__${(doctorProfile.doctorId||"").trim()}`;
function ensureDoctorDocs(){
  const k=doctorKey();
  state.docsByDoctor[k]=state.docsByDoctor[k]||{symptom:[],reply:[],other:[]};
  return state.docsByDoctor[k];
}
...
function billingMapCode(m){
  const t=m?.tokutei01_name||"", p=m?.product_name||"";
...
function exportItemsCSV(items, filename){
  ...
  for(const it of items){
    const base={
      id:it.id,date:it.date,confirmedAt:it.confirmedAt,status:it.status,
      patientId:it.patientId,patient:patientLabel(it.patientId),
      operatorId:it.operatorId,operator:operatorLabel(it.operatorId),
      procedureId:it.procedureId,procedure:procedureLabel(it.procedureId),
      assignedDoctorId:it.assignedDoctorId,assignedDoctor:doctorLabelById(it.assignedDoctorId),
      approved_by:it.approved_by||"", approvedBy:doctorLabelById(it.approved_by||""),
      approved_at:it.approved_at||"", doctor_comment:it.doctor_comment||""
    };
    const mats=Array.isArray(it.materials)?it.materials:[];
    if(!mats.length){
      rows.push({...base,qty:"",mat_product_name:"",mat_product_no:"",mat_product_sta:"",mat_tokutei01_name:"",
      ...
      for(const m of mats){
        rows.push({...base,
          ...
          mat_jan13:m.jan13||"",mat_gtin14:m.gtin14||"",
  ...
  const headers=[
    ...
    "assignedDoctorId","assignedDoctor","approved_by","approvedBy","approved_at","doctor_comment",
    "qty","mat_product_name","mat_product_no","mat_product_sta","mat_tokutei01_name","mat_total_reimbursement_price_yen",
...
const setView=(h)=>{ location.hash=`#${h}`; };
...
/* ========= summary (field only) ========= */
const totalQty=()=> (scanCtx?.materials||[]).reduce((p,m)=>p+Number(m.qty||1),0);
function updateSummaryUI(){
  const host=$("#summaryHost"); if(!host) return;
  const v=view();
  const isField=(role==="field") && (v.startsWith("/field") || v==="/" || v==="");
  if(!isField){ host.style.display="none"; host.innerHTML=""; return; }

  ensureScanCtx();
  const opMissing=!scanCtx.operatorId, ptMissing=!scanCtx.patientId, prMissing=!scanCtx.procedureId;
...
/* ========= suggestion (after material scan) ========= */
const normalizeText=(s)=>String(s||"").toLowerCase();
const procedureLabelSafe=(id)=>PROCEDURES.find(p=>p.id===id)?.label || id;
...
function computeProcedureSuggestions(){
  ...
  for(const rule of rules){
    ...
    const strength=1+Math.min(2,hits.length-1);
  ...
  return Array.from(agg.entries())
    .map(([pid,v])=>({id:pid,label:procedureLabelSafe(pid),score:v.score,reason:Array.from(v.reasons).slice(0,3).join(" / ")}))
...
const buildJanPath=(jan13)=>`./dict_jan/${jan13.slice(0,3)}/${jan13.slice(0,4)}.csv`;
const buildGtinPath=(gtin14)=>`./gtin_index/${gtin14.slice(0,3)}/${gtin14.slice(0,4)}.csv`;
...
async function lookupByJan13(jan13){
  ...
  }catch(e){ return {status:"dict_fetch_error",error:e.message}; }
}
...
function mergeOrAddMaterial(list, item){
  ...
  if(idx>=0){ list[idx].qty=Number(list[idx].qty||1)+1; return list[idx]; }
  item.qty=1; list.unshift(item); return item;
}
function decMaterialById(list,id){
  const i=list.findIndex(x=>x.id===id); if(i<0) return;
  const q=Number(list[i].qty||1);
  if(q>1) list[i].qty=q-1; else list.splice(i,1);
}
...
async function handleDetected(raw){
  ...
  if(candidate.code===codeKey && (now-candidate.ts)<=DOUBLE_HIT_WINDOW_MS){
    candidate.count+=1; candidate.ts=now;
  }else{
  ...
  lastScan.anyTs=t;
  if(codeKey===lastScan.raw) lastScan.sameTs=t; else { lastScan.raw=codeKey; lastScan.sameTs=t; }
  ...
  const item={
    id:uid("MAT"),
    raw:String(raw||""),
    jan13:null, gtin14:null,
    dict_status:"unknown",
    product_name:"", product_no:"", product_sta:"",
  ...
  const sub=updated.tokutei01_name?`${updated.tokutei01_name} / ×${updated.qty||1}`:`×${updated.qty||1}`;
...
function screenDoctorLogin(){
  const deptOptions=[`<option value="">選択</option>`]
    .concat(doctorDeptList().map(d=>`<option value="${d}"${doctorProfile.dept===d?" selected":""}>${d}</option>`)).join("");
  const list=DOCTORS.filter(d=>!doctorProfile.dept||d.dept===doctorProfile.dept).slice()
    .sort((a,b)=>(a.name||"").localeCompare(b.name||"","ja"));
  const docOptions=[`<option value="">選択</option>`]
    .concat(list.map(d=>`<option value="${d.id}"${doctorProfile.doctorId===d.id?" selected":""}>${d.name}（${d.id}）</option>`)).join("");
...
function screenDoctorApprovals(){
  ...
  const list = pending.length ? pending.map(x=>`
    ...
    </div>`).join("") : `<div class="muted">承認待ちなし</div>`;
...
function screenDrafts(){
  const list = state.drafts.length ? state.drafts.map(d=>{
    ...
    const qty=(d.materials||[]).reduce((p,m)=>p+Number(m.qty||1),0);
    return `<div class="listItem">
      <div><b>${patientLabel(d.patientId)}${mode}</b>
        <div class="muted">${operatorLabel(d.operatorId)} / ${procedureLabel(d.procedureId)} / ${qty}点</div>
      </div>
...
function screenDone(){
  ...
  const list = items.length ? items.map(x=>{
    ...
    const qty=(x.materials||[]).reduce((p,m)=>p+Number(m.qty||1),0);
...
function screenApproverSelect(){
  ...
  const deptOptions=[`<option value="ALL"${scanCtx.approverDept==="ALL"?" selected":""}>すべて</option>`]
    .concat(doctorDeptList().map(d=>`<option value="${d}"${scanCtx.approverDept===d?" selected":""}>${d}</option>`)).join("");
...
function renderBillingDetail(item){
  ...
  const comment=item.doctor_comment?`
    <div style="border:1px solid #f2d2dd;border-radius:16px;padding:10px;background:#fff;margin-top:10px;">
      <div class="h2">医師コメント</div><div class="muted">${item.doctor_comment}</div>
    </div>`:"";
